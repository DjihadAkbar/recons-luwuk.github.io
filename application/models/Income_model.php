...
class Income_model extends CI_Model
{
        ...
        COUNT(case when trips.trip != 1 then 1 END) as "Jumlah Trip", route, routes.id,
                ...
                (
                    ...
                    SUM(
                        ...
                        FROM entry_data as entry_b
                        JOIN rate ON routes.id = rate.id_route AND entry_b.date >= rate.start_date and entry_b.rate_type = rate.rate_type
                        WHERE MONTHNAME(entry_b.DATE) = "' . $lastMonth . '" AND YEAR(entry_b.DATE) = "' . $lastYear . '" and entry_a.id_route = entry_b.id_route AND entry_a.id_ferry = entry_b.id_ferry
                        GROUP BY ferry
                    ) AS totalLastYear,
                    (
    ...
    public function incomePerHarbour()
    {
        ...
        if ($this->session->userdata('logged_in'))
            $pelabuhan = $this->session->userdata['pelabuhan'];
        $this->db->select('ferry,monthname(entry_a.date) as month_date,entry_a.date as date,harbour, 
        COUNT(case when trips.trip != 1 then 1 END) as "Jumlah Trip", route, routes.id,
                SUM(
                (rate.Gol1 * entry_a.Gol1) + 
                (rate.Gol2 * entry_a.Gol2) +
                ...
                (
                    ...
                    SUM(
                        ...
                        FROM entry_data as entry_b
                        JOIN rate ON routes.id = rate.id_route AND entry_b.date >= rate.start_date and entry_b.rate_type = rate.rate_type
                        WHERE MONTHNAME(entry_b.DATE) = "' . $lastMonth . '" AND YEAR(entry_b.DATE) = "' . $lastYear . '" AND entry_a.id_harbour = entry_b.id_harbour
                        GROUP BY harbour
                    ) AS totalLastYear,
                    (
                        SELECT COUNT(case when trips.trip != 1 then 1 END)

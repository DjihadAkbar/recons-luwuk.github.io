        ...
        COUNT(case when trips.trip != 1 then 1 END) as "Jumlah Trip", route, routes.id, 
                ...
                (
                    ...
                    (
                        SELECT sum(trip)
                        FROM harbour_target
                        where entry_a.id_ferry = harbour_target.id_ferry AND entry_a.id_harbour = harbour_target.id_harbour AND entry_a.id_route = harbour_target.id_route AND monthname(entry_a.date) = harbour_target.month  AND year(entry_a.date) = harbour_target.year
        ...
        COUNT(case when trips.trip != 1 then 1 END) as "Jumlah Trip", route, routes.id,
                ...
                (
                    ...
                    (
                        SELECT sum(trip)
                        FROM harbour_target
                        where entry_a.id_harbour = harbour_target.id_harbour AND entry_a.id_harbour = harbour_target.id_harbour AND entry_a.id_route = harbour_target.id_route AND monthname(entry_a.date) = harbour_target.month  AND year(entry_a.date) = harbour_target.year 
                        GROUP BY id_harbour
                    ) as target_trip,
                    (
                        SELECT sum(target)
